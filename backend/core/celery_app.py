...
class DesktopAwareTask(celery_app.Task):
    ...
    def update_state(self, task_id=None, state=None, meta=None, **kwargs):
        """桌面模式下 update_state 会写 Celery result backend（Redis），

        而桌面安装包没有 Redis → 每次调用都抛 ConnectionError，任务直接失败。
        真正的进度是另走「写库供前端轮询」的机制，Celery 自带状态桌面端用不到，
        所以这里直接降级为日志，不触碰 backend。生产模式行为不变。
        """
        if _is_desktop_mode():
            import logging
            msg = (meta or {}).get("message") if isinstance(meta, dict) else None
            progress = (meta or {}).get("progress") if isinstance(meta, dict) else None
            logging.getLogger(__name__).info(
                f"[desktop][{self.name}] state={state} progress={progress} {msg or ''}"
            )
            return
